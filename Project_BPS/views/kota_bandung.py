...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kota Bandung</h2>
...
#===============================
                try:
                    df.to_sql("uploaded_kota_bandung", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kota_bandung'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kota_bandung", conn)
    else:
    ...
    m = folium.Map(location=[-6.917932890656802, 107.62152724116562], tiles='OpenStreetMap', zoom_start=13)
...
df_bandung = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_bandung.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_bandung.csv")
...
df.to_sql("kota_bandung", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kota_bandung")
