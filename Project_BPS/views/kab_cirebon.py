...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Cirebon</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Cirebon", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_cirebon", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_cirebon'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_cirebon", conn)
    else:
    ...
    m = folium.Map(location=[-6.809682550421537, 108.55095606240589], tiles='OpenStreetMap', zoom_start=13)
...
df_cirebon = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_cirebon.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_cirebon.csv")
...
df.to_sql("kabupaten_cirebon", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Cirebon")
...
cursor.execute("SELECT * FROM kabupaten_cirebon")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_cirebon['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_cirebon['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_cirebon['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_cirebon['idsubsls'].nunique()
