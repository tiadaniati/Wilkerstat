...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_bekasi", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_bekasi'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_bekasi", conn)
    else:
    ...
    m = folium.Map(location=[-6.257453425627817, 107.11859963221147], tiles='OpenStreetMap', zoom_start=13)
...
df_bekasi = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bekasi.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bekasi.csv")
...
df.to_sql("kabupaten_bekasi", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_bekasi")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_bekasi['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_bekasi['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_bekasi['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_bekasi['idsubsls'].nunique()
