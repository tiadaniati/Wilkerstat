...
# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="Kabupaten Bekasi", page_icon="🏘", layout="wide")
st.markdown("""
    <h1 style='text-align: center; font-size: 60px;'>🔍 Monitoring Wilkerstat Sensus Ekonomi 2026</h1>
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Bekasi</h2>
        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kab_bekasi", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_bekasi;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_bekasi.geojson'
    try:
    ...
    m = folium.Map(
        location=[-6.257453425627817, 107.11859963221147],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_kabbekasi_csv = pd.read_csv("Project_BPS/dataset/kab_bekasi.csv")
except FileNotFoundError:
...
st.title("Database Kabupaten Bekasi")

df_ref = df_kabbekasi_csv.copy()
...
try:
    df_ref.to_sql("kab_bekasi", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kab_bekasi' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_bekasi;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_bekasi' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kabupaten Bekasi", conn_st.engine)
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_kabbekasi_csv['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_kabbekasi_csv['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_kabbekasi_csv['idsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_kabbekasi_csv['idsls'].nunique()
