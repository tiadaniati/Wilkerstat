        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kab_majalengka", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_majalengka;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_majalengka.geojson'
    try:
    ...
    m = folium.Map(
        location=[-6.8112502833817885, 108.27580690031157],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kab_majalengka.csv")
except FileNotFoundError:
...
try:
    df_ref.to_sql("kab_majalengka", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kab_majalengka' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_majalengka;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_majalengka' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kabupaten Majalengka", conn_st.engine)
...
with stat1:
    ...
    st.markdown(metric_card("Jumlah SLS", df_csv['idsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_csv['idsls'].nunique()
