...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Majalengka</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Majalengka", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_majalengka", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_majalengka'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_majalengka", conn)
    else:
    ...
    m = folium.Map(location=[-6.8112502833817885, 108.27580690031157], tiles='OpenStreetMap', zoom_start=13)
...
df_majalengka = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_majalengka.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_majalengka.csv")
...
df.to_sql("kabupaten_majalengka", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Majalengka")
...
cursor.execute("SELECT * FROM kabupaten_majalengka")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_majalengka['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_majalengka['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_majalengka['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_majalengka['idsubsls'].nunique()
