...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Sumedang</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Sumedang", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_sumedang", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_sumedang'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_sumedang", conn)
    else:
    ...
    m = folium.Map(location=[-6.792819374544142, 107.96125926479975], tiles='OpenStreetMap', zoom_start=13)
...
df_sumedang = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_sumedang.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_sumedang.csv")
...
df.to_sql("kabupaten_sumedang", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Sumedang")
...
cursor.execute("SELECT * FROM kabupaten_sumedang")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_sumedang['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_sumedang['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_sumedang['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_sumedang['idsubsls'].nunique()
