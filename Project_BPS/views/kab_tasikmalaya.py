...
with col1:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_tasikmalaya.geojson'
    try:
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kab_tasikmalaya.csv")
except FileNotFoundError:
...
try:
    df_ref.to_sql("kab_tasikmalaya", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kab_tasikmalaya' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_tasikmalaya;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_tasikmalaya' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kabupaten Tasikmalaya", conn_st.engine)
