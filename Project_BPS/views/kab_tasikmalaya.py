...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Tasikmalaya</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Tasikmalaya", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_tasikmalaya", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_tasikmalaya'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_tasikmalaya", conn)
    else:
    ...
    m = folium.Map(location=[-7.452059791908823, 108.12532960444605], tiles='OpenStreetMap', zoom_start=13)
...
df_tasikmalaya = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_tasikmalaya.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_tasikmalaya.csv")
...
df.to_sql("kabupaten_tasikmalaya", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_Tasikmalaya")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_tasikmalaya['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_tasikmalaya['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_tasikmalaya['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_tasikmalaya['idsubsls'].nunique()
