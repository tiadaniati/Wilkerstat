...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Subang</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Subang", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_subang", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_subang'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_subang", conn)
    else:
    ...
    m = folium.Map(location=[-6.451650169941391, 107.7339048721714], tiles='OpenStreetMap', zoom_start=13)
...
df_subang = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_subang.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_subang.csv")
...
df.to_sql("kabupaten_subang", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Subang")
...
cursor.execute("SELECT * FROM kabupaten_subang")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_subang['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_subang['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_subang['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_subang['idsubsls'].nunique()
