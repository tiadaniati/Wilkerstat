...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Karawang</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Karawang", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_karawang", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_karawang'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_karawang", conn)
    else:
    ...
    m = folium.Map(location=[-6.298829639855076, 107.38014536808956], tiles='OpenStreetMap', zoom_start=13)
...
df_karawang = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_karawang.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_karawang.csv")
...
df.to_sql("kabupaten_karawang", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Karawang")
...
cursor.execute("SELECT * FROM kabupaten_karawang")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_karawang['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_karawang['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_karawang['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_karawang['idsubsls'].nunique()
