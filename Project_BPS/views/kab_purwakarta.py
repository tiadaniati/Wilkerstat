...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Purwakarta</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Purwakarta", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_purwakarta", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_purwakarta'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_purwakarta", conn)
    else:
    ...
    m = folium.Map(location=[-6.582820866383887, 107.41403764871238], tiles='OpenStreetMap', zoom_start=13)
...
df_purwakarta = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_purwakarta.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_purwakarta.csv")
...
df.to_sql("kabupaten_purwakarta", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Purwakarta")
...
cursor.execute("SELECT * FROM kabupaten_purwakarta")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_purwakarta['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_purwakarta['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_purwakarta['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_purwakarta['idsubsls'].nunique()
