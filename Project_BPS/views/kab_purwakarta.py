        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kab_purwakarta", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_purwakarta;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_purwakarta.geojson'
    try:
    ...
    m = folium.Map(
        location=[-6.582820866383887, 107.41403764871238],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kab_purwakarta.csv")
except FileNotFoundError:
...
try:
    df_ref.to_sql("kab_purwakarta", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kab_purwakarta' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_purwakarta;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_purwakarta' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kabupaten Purwakarta", conn_st.engine)
