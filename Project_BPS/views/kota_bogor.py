        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kota_bogor", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kota_bogor;", ttl=600)
            return df_query
        except Exception as e:
    ...
    m = folium.Map(
        location=[-6.596891982407816, 106.80390437592699],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kota_bogor.csv")
except FileNotFoundError:
...
try:
    ...
    st.warning(f"Gagal me-refresh data 'kota_bogor' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kota_bogor;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kota_bogor' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kota Bogor", conn_st.engine)
