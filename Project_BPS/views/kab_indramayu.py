...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Indramayu</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Indramayu", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_indramayu", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_indramayu'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_indramayu", conn)
    else:
    ...
    m = folium.Map(location=[-6.43979794394636, 108.16713461413325], tiles='OpenStreetMap', zoom_start=13)
...
df_indramayu = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_indramayu.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_indramayu.csv")
...
df.to_sql("kabupaten_indramayu", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Indramayu")
...
cursor.execute("SELECT * FROM kabupaten_indramayu")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_indramayu['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_indramayu['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_indramayu['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_indramayu['idsubsls'].nunique()
