...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_bandung_barat", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_bandung_barat'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_bandung_barat", conn)
    else:
    ...
    m = folium.Map(location=[-6.880013456237037, 107.41851262909165], tiles='OpenStreetMap', zoom_start=13)
...
df_bandung_barat = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bandung barat.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bandung barat.csv")
...
df.to_sql("kabupaten_bandung_barat", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_bandung_barat")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_bandung_barat['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_bandung_barat['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_bandung_barat['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_bandung_barat['idsubsls'].nunique()
