    ...
    if fl is not None:
        ...
        if st.session_state.get('uploaded_filename') != filename:
            ...
            try:
                ...
                try:
                    df.to_sql("uploaded_kab_bandungbrt", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with stat1:
    ...
    st.markdown(metric_card("Jumlah SLS", df_bandungbrt_csv['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_bandungbrt_csv['idsubsls'].nunique()
