...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Kuningan</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Kuningan", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_kuningan", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_kuningan'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_kuningan", conn)
    else:
    ...
    m = folium.Map(location=[-6.991273156876795, 108.56497165609984], tiles='OpenStreetMap', zoom_start=13)
...
df_kuningan = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_kuningan.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_kuningan.csv")
...
df.to_sql("kabupaten_kuningan", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Kuningan")
...
cursor.execute("SELECT * FROM kabupaten_kuningan")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_kuningan['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_kuningan['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_kuningan['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_kuningan['idsubsls'].nunique()
