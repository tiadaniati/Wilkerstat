...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_sukabumi;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_sukabumi.geojson'
    try:
    ...
    m = folium.Map(
        location=[-6.850939167655336, 106.95856563846927],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kab_sukabumi.csv")
except FileNotFoundError:
...
try:
    df_ref.to_sql("kab_sukabumi", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kab_sukabumi' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_sukabumi;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_sukabumi' dari DB: {e}")
