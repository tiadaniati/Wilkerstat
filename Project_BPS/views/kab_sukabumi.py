...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Sukabumi</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Sukabumi", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_sukabumi", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_sukabumi'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_sukabumi", conn)
    else:
    ...
    m = folium.Map(location=[-6.936841717262919, 106.92726533186152], tiles='OpenStreetMap', zoom_start=13)
...
df_sukabumi = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_sukabumi.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_sukabumi.csv")
...
df.to_sql("kabupaten_sukabumi", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_sukabumi")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_sukabumi['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_sukabumi['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_sukabumi['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_sukabumi['idsubsls'].nunique()
