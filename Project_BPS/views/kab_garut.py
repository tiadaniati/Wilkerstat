        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kab_garut", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_garut;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kab_garut.geojson'
    try:
...
try:
    ...
    st.warning(f"Gagal me-refresh data 'kab_garut' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kab_garut;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kab_garut' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kabupaten Garut", conn_st.engine)
