...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Garut</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Garut", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_garut", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_garut'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_garut", conn)
    else:
    ...
    m = folium.Map(location=[-7.336394663564128, 107.7671033388639], tiles='OpenStreetMap', zoom_start=13)
...
df_garut = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_garut.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_garut.csv")
...
df.to_sql("kabupaten_garut", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_garut")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_garut['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_garut['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_garut['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_garut['idsubsls'].nunique()
