...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_ciamis", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_ciamis'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_ciamis", conn)
    else:
    ...
    m = folium.Map(location=[-7.2568016201885355, 108.40894841925659], tiles='OpenStreetMap', zoom_start=13)
...
df_ciamis = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_ciamis.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_ciamis.csv")
...
df.to_sql("kabupaten_ciamis", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_ciamis")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_ciamis['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_ciamis['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_ciamis['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_ciamis['idsubsls'].nunique()
