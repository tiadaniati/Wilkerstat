...
#===============================
                try:
                    df.to_sql("uploaded_kota_banjar", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kota_banjar'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kota_banjar", conn)
    else:
...
df_banjar = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_banjar.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_banjar.csv")
...
df.to_sql("kota_banjar", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kota_banjar")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_banjar['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_banjar['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_banjar['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_banjar['idsubsls'].nunique()
