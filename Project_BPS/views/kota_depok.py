        ...
        if fl is not None:
            ...
            if st.session_state.get('uploaded_filename') != filename:
                ...
                try:
                    ...
                    try:
                        df.to_sql("uploaded_kota_depok", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kota_depok;", ttl=600)
            return df_query
        except Exception as e:
    ...
    geojson_file = 'Project_BPS/Geolocation/map_kota_depok.geojson'
    try:
    ...
    m = folium.Map(
        location=[-6.4018306401127925, 106.7985675054929],
        zoom_start=13,
        tiles='OpenStreetMap'
    )
...
try:
    df_csv = pd.read_csv("Project_BPS/dataset/kota_depok.csv")
except FileNotFoundError:
...
try:
    df_ref.to_sql("kota_depok", con=conn_st.engine, if_exists="replace", index=False, dtype={
    ...
    st.warning(f"Gagal me-refresh data 'kota_depok' di DB: {e}")

try:
    df = conn_st.query("SELECT * FROM kota_depok;", ttl=600)
    ...
    st.error(f"Gagal mengambil data 'kota_depok' dari DB: {e}")
...
update_rekap_total_landmark(df_merged, "Kota Depok", conn_st.engine)
