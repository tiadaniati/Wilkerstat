...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kota Depok</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kota Depok", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kota_depok", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kota_depok'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kota_depok", conn)
    else:
    ...
    m = folium.Map(location=[-6.4018306401127925, 106.7985675054929], tiles='OpenStreetMap', zoom_start=13)
...
df_depok = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_depok.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_depok.csv")
...
df.to_sql("kota_depok", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kota_depok")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_depok['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_depok['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_depok['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_depok['idsubsls'].nunique()
