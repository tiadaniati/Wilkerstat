...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Bogor</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Bogor", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_bogor", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_bogor'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_bogor", conn)
    else:
    ...
    m = folium.Map(location=[-6.60127146542485, 106.81133614182369], tiles='OpenStreetMap', zoom_start=13)
...
df_bogor = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bogor.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_bogor.csv")
...
df.to_sql("kabupaten_bogor", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kabupaten_bogor")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_bogor['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_bogor['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_bogor['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_bogor['idsubsls'].nunique()
