    ...
    if fl is not None:
        ...
        if st.session_state.get('uploaded_filename') != filename:
            ...
            try:
                ...
                try:
                    df.to_sql("uploaded_kab_bogor", con=conn_st.engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    def fetch_uploaded_data():
        ...
        try:
            df_query = conn_st.query("SELECT * FROM uploaded_kab_bogor;", ttl=600)
            return df_query
        except Exception as e:
...
try:
    df_kabbogor_csv = pd.read_csv("Project_BPS/dataset/kab_bogor.csv")
except FileNotFoundError:
...
try:
    ...
    st.warning(f"Gagal me-refresh data 'kab_bogor' di DB: {e}")
...
with stat1:
    ...
    st.markdown(metric_card("Jumlah SLS", df_kabbogor_csv['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_kabbogor_csv['idsubsls'].nunique()
