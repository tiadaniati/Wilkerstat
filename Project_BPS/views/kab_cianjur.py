...
st.markdown("""
    ...
    <h2 style='text-align: center; font-size: 50px;'>Kabupaten Cianjur</h2>
""", unsafe_allow_html=True)
st.set_page_config(page_title="Kabupaten Cianjur", page_icon="🌳", layout="wide")
#=========================================================================
...
#===============================
                try:
                    df.to_sql("uploaded_kabupaten_cianjur", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kabupaten_cianjur'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kabupaten_cianjur", conn)
    else:
    ...
    m = folium.Map(location=[-7.076432096964623, 107.12763311753632], tiles='OpenStreetMap', zoom_start=13)
...
df_cianjur = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_cianjur.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kab_cianjur.csv")
...
df.to_sql("kabupaten_cianjur", con=engine, if_exists="replace", index=False, dtype={
...
st.title("Database Kabupaten Cianjur")
...
cursor.execute("SELECT * FROM kabupaten_cianjur")
...
with stat1:
    st.markdown(metric_card("Jumlah Kecamatan", df_cianjur['nmkec'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah Desa", df_cianjur['nmdesa'].nunique()), unsafe_allow_html=True)
    st.markdown(metric_card("Jumlah SLS", df_cianjur['idsubsls'].nunique()), unsafe_allow_html=True)
...
with stat2:
    ...
    jumlah_sls = df_cianjur['idsubsls'].nunique()
