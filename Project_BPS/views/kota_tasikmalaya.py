...
#===============================
                try:
                    df.to_sql("uploaded_kota_tasikmalaya", con=engine, if_exists=if_exists_option, index=False, dtype={
...
with col1:
    ...
    if table_exists(conn, 'uploaded_kota_tasikmalaya'):
        df_uploaded = pd.read_sql("SELECT * FROM uploaded_kota_tasikmalaya", conn)
    else:
    ...
    m = folium.Map(location=[-7.349078519766239, 108.22107611791895], tiles='OpenStreetMap', zoom_start=13)
...
df_tasikmalaya = pd.read_csv("/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_tasikmalaya.csv")
...
# Baca file CSV
df = pd.read_csv(r"/Users/jibrilnikki/Documents/Code/Project_BPS/dataset/kota_tasikmalaya.csv")
...
df.to_sql("kota_tasikmalaya", con=engine, if_exists="replace", index=False, dtype={
...
cursor.execute("SELECT * FROM kota_tasikmalaya")
